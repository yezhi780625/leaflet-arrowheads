  ...
  buildVectorHats: function (options) {
    ...
    this._parts.forEach((peice, index) => {
      ...
      if (!isNaN(frequency)) {
        ...
        frequency.toString().slice(frequency.toString().length - 1, frequency.toString().length) ===
        '%'
      ) {
        console.log(
        ...
        frequency.toString().slice(frequency.toString().length - 2, frequency.toString().length) ===
        'px'
      ) {
        spacing = (() => {
      ...
      if (options.frequency === 'allvertices') {
        derivedBearings = (() => {
          ...
          for (var i = 1; i < latlngs.length; i++) {
            let bearing =
              L.GeometryUtil.angle(this._map, latlngs[modulus(i - 1, latlngs.length)], latlngs[i]) +
              180;
      ...
      } else if (options.frequency === 'startend') {
        derivedLatLngs = [latlngs[latlngs.length - 1], latlngs[0]];
        ...
        derivedBearings = [
          ...
          L.GeometryUtil.angle(this._map, latlngs[1], latlngs[0]) + 180,
        ];
      } else {
        derivedLatLngs = [];
